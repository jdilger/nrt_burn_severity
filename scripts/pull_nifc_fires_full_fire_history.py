...
#subset columns
gdf_finalv1 = gdf_yrv1[['OBJECTID', 'Name', 'Acres', 'Discovery','start', 'end', 'pre_start', 'pre_end', 'geometry']]
# write to shp
...
#subset columns
gdf_finalv2 = gdf_yrv2[['OBJECTID', 'Name', 'Acres', 'Discovery','start', 'end', 'pre_start', 'pre_end', 'geometry']]
...
#subset columns
gdf_finalv3 = gdf_yrv3[['OBJECTID', 'Name', 'Acres', 'Discovery','start', 'end', 'pre_start', 'pre_end', 'geometry']]
# write to shp
...
# %%
#%%
# pull in calfire 2020 fires subset
calfire_gdf = gpd.read_file(r'C:\FireFactor\T2Fuels\nrt_burn_severity\data\shp\calfire_2020fires_notIn_nifc_fires.shp')
#calfire_gdf = calfire_gdf.to_crs(spatialref) # set to same crs as nifc fires gdf
calfire_gdf = calfire_gdf.rename(columns = {'FIRE_NAME': 'Name',
                                            'GIS_ACRES': 'Acres',
                                            'ALARM_DATE': 'Discovery'})

calfire_gdf.loc[:,'OBJECTID'] = calfire_gdf.index
# calfire_gdf = calfire_gdf[['OBJECTID', 'Name', 'Acres', 'Discovery', 'geome']]
calfire_gdf.loc[:,'Discovery'] = pd.to_datetime(calfire_gdf.loc[:,'Discovery'])
...
calfire_gdf.loc[:,'start'] = calfire_gdf['Discovery'] + timedelta(days= 275)      # 1yr later and 90 days before
calfire_gdf.loc[:,'end'] = calfire_gdf['Discovery'] + timedelta(days = 365)       # 1yr later
calfire_gdf.loc[:,'pre_start'] = calfire_gdf['Discovery'] - timedelta(days=455)   # 1yr earlier and 90 days before
calfire_gdf.loc[:,'pre_end'] = calfire_gdf['Discovery'] - timedelta(days=365)     # 1yr earlier
...
post_end_cut_off = pd.to_datetime(datetime.now()) - timedelta(days=5)
# exclude fire records whose 'end' value is beyond the cutoff
before_cutoff = len(calfire_gdf)
calfire_gdf = calfire_gdf[calfire_gdf.end <= post_end_cut_off]
after_cutoff = len(calfire_gdf)
print(f'removing {before_cutoff - after_cutoff} fires requiring future non-existent imagery')
# convert datetime to string dtype
calfire_gdf.loc[:,['Discovery','start', 'end', 'pre_start', 'pre_end']] = calfire_gdf[['Discovery','start', 'end', 'pre_start', 'pre_end']].astype('str')

# convert to yyyy-mm-dd format
calfire_gdf.loc[:, 'start'] = [x.split(' ')[0] for x in calfire_gdf['start']]
calfire_gdf.loc[:, 'end'] = [x.split(' ')[0] for x in calfire_gdf['end']]
calfire_gdf.loc[:, 'pre_start'] = [x.split(' ')[0] for x in calfire_gdf['pre_start']]
calfire_gdf.loc[:, 'pre_end'] = [x.split(' ')[0] for x in calfire_gdf['pre_end']]
calfire_gdf.loc[:, 'Discovery'] = [x.split(' ')[0] for x in calfire_gdf['Discovery']]
#subset columns
calfire_gdf_final = calfire_gdf[['OBJECTID', 'Name', 'Acres', 'Discovery','start', 'end', 'pre_start', 'pre_end', 'geometry']]

# merge nifc final gdf with calfire final gdf and export as one shapefile
#gdf_nifc_plus_calfire = gpd.GeoDataFrame(pd.concat([gdf_finalv3, calfire_gdf_final]))
# write to shp
file_name = f'calfire_frap_fires_{year_choice}_gte500acres.shp'
out_file = f'{os.path.join(out_path,file_name)}'
print(f'writing out to {out_file}')
calfire_gdf_final.to_file(out_file)
# %%
